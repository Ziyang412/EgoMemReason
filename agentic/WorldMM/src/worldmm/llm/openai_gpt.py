...
class OpenAIModel:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.use_azure:
            api_key = azure_key or api_key or os.getenv("OPENAI_API_KEY")
            if not api_key:
                raise OpenAIModelError("Azure API key not found. Set AZURE_OPENAI_KEY environment variable.")
            try:
                ...
                raise OpenAIModelError("Failed to initialize Azure OpenAI client") from e
            # For Azure, model_name is the deployment name
            self.model_name = MODEL_DICT[model_name]
            logger.info(f"Using Azure OpenAI: endpoint={azure_endpoint}, deployment={self.model_name}")
        else:
            # Standard OpenAI
    ...
    def _process_content(self, content: Union[str, Dict[str, Any], List[Dict[str, Any]]]) -> None:
        ...
        for i, item in enumerate(content):
            ...
            if t == "text" and "text" in item:
                if self.use_azure:
                    # Azure chat completions API uses {"type": "text", "text": ...}
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, media_type, result in results:
                if media_type == "error":
                    ...
                    if self.use_azure:
                        image_items = [{"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{frame}"}}
                                       for frame in result]
                    else:
    ...
    async def _generate_single_prompt(self, prompt: Union[str, List[Dict[str, Any]]], text_format: Optional[type] = None) -> Any:
        ...
        try:
            ...
            if self.use_azure:
                azure_kwargs = dict(self.kwargs)
                if text_format is not None:
                    azure_kwargs["response_format"] = {"type": "json_object"}
