...
{r['retrieved_content']}"""
    ...
    def _render_retrieved_items_for_qa(
        ...
        Args:
            ...
            max_images: Maximum total images to include (Azure limit is 50)
    ...
    def retrieve_from_visual(
        ...
        # Retrieve from visual memory (max_frames=45 to stay under Azure's 50-image limit)
...
Step 2 (only if search): Pick one memory type (episodic/semantic/visual) and form a search query."""
        ...
        if choices:
            option_letters = ", ".join(sorted(choices.keys()))
            qa_content.append({
                "type": "text",
                "text": f"\nPlease provide only the final answer from the choices given (e.g., {option_letters})."
            })
