...
class VisualMemory:
    ...
    def _retrieve_by_similarity(
        ...
        if as_context:
            ...
            # Each clip is a short video file (~30sec), so extract frames from
            # the entire file (don't use clip_start_sec/clip_end_sec which are
            # absolute timestamps, not offsets within the video file).
