...
def infer_text_only(
    ...
    with torch.inference_mode():
        try:
            ...
            output = model.chat(tokenizer, None, prompt_text, generation_config)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="InternVL text-only evaluation for generic benchmark query JSON"
    )
    parser.add_argument(
        "--input_json",
        type=str,
    ...
    parser.add_argument(
        "--output_json",
        type=str,
        default="/nas-ssd2/ziyang/Memory_project/COLM/evaluation/InternVL/results/temporal_ordering/temporal_ordering_60_current_format_text_only_internvl3_5_8b.json",
    )
    ...
    if auto_model is None:
        raise ImportError("transformers.AutoModel is not available in this environment.")
    if auto_tokenizer is None:
        raise ImportError("transformers.AutoTokenizer is not available in this environment.")
    ...
    try:
        model = auto_model.from_pretrained(args.model_name, **load_kwargs).eval()
    except TypeError as exc:
        # Some InternVL snapshots may not accept use_flash_attn; retry without it.
        if "use_flash_attn" in load_kwargs and "use_flash_attn" in str(exc):
            load_kwargs.pop("use_flash_attn", None)
            model = auto_model.from_pretrained(args.model_name, **load_kwargs).eval()
        else:
            raise

    tokenizer = auto_tokenizer.from_pretrained(args.model_name, trust_remote_code=True, use_fast=False)
    if not hasattr(model, "chat"):
        raise AttributeError(
            f"Loaded model {args.model_name!r} does not expose a .chat(...) API required by this evaluator."
        )
