...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run InternVL3.5-8B on EgoLife-style frame QA datasets."
    )
    ...
    for load_kwargs in load_candidates:
        ...
        try:
            ...
            # InternVL3.5 remote code can fail under meta init path.
    ...
    if max_context_len is not None and image_tokens_per_frame > 0:
        ...
        if args.vision_batch_size and args.vision_batch_size > 0:
            if args.vision_batch_size > safe_frame_limit:
                print(
                    f"[context] vision_batch_size={args.vision_batch_size} exceeds estimated safe limit "
                    f"{safe_frame_limit}; capping vision_batch_size to {safe_frame_limit}."
                )
                args.vision_batch_size = safe_frame_limit
            else:
                print(
                    f"[context] using chunk summarization: max_frames={args.max_frames}, "
                    f"vision_batch_size={args.vision_batch_size}, safe_chunk_limit={safe_frame_limit}."
                )
        elif args.max_frames > safe_frame_limit:
            print(
