#!/usr/bin/env bash
# Frame-count ablation on final_benchmark_500_apr22.json:
#   8B: 16 / 32 / 64 / 128 / 256f  -> 8 jobs each (1 GPU per job)
#   8B: 512f                       -> 4 jobs (2 GPUs per job)
#   8B: 1024f                      -> 2 jobs (4 GPUs per job)
#   32B: 256f                      -> 1 job (all 8 GPUs)
#   Then hold GPUs (one 8B copy per GPU).
set -euo pipefail
...
GPUS_8x1="0 1 2 3 4 5 6 7"       # 8 jobs, 1 GPU each
GPUS_4x2="0,1 2,3 4,5 6,7"       # 4 jobs, 2 GPUs each
GPUS_2x4="0,1,2,3 4,5,6,7"       # 2 jobs, 4 GPUs each
GPUS_1x8="0,1,2,3,4,5,6,7"       # 1 job,  8 GPUs

# --- Stage 1: 8B frame-count ablation ---
for f in 16 32 64 128 256; do
  run_stage "apr22_8b_${f}f"   "Qwen/Qwen3-VL-8B-Instruct"  "$f"   "$GPUS_8x1"
done
run_stage   "apr22_8b_512f"    "Qwen/Qwen3-VL-8B-Instruct"  512    "$GPUS_4x2"
run_stage   "apr22_8b_1024f"   "Qwen/Qwen3-VL-8B-Instruct"  1024   "$GPUS_2x4"

# --- Stage 2: 32B at 256f ---
run_stage   "apr22_32b_256f"   "Qwen/Qwen3-VL-32B-Instruct" 256    "$GPUS_1x8"
