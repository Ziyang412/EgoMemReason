#!/usr/bin/env bash
# Qwen3-VL-8B 256f on final_benchmark_500_apr22.json with per-split instructions.
# GPUs 0-3 (4 chunks in parallel). Holds GPUs 0-3 after eval finishes.
set -euo pipefail
...
EXP_NAME="apr22_8b_per_split_instr_${MAX_FRAMES}f"
...
echo "=== Qwen3-VL-8B per-split instructions on GPUs ${GPU_GROUPS} ==="
echo "Input: ${INPUT_JSON}"
echo "Output dir: ${OUTPUT_DIR}"

# Chunk dataset into NUM_WORKERS shards
echo "Chunking..."
...
# Merge
echo "Merging..."
$PYTHON_BIN - "$OUTPUT_DIR" "$INPUT_BASE" "$NUM_WORKERS" <<'PY'
import json, sys
...
if [[ "$fail" -ne 0 ]]; then
  echo "Some workers failed; still proceeding to hold GPUs."
fi

echo ""
echo "=== Eval done. Holding GPUs ${GPU_GROUPS} (Ctrl+C to release) ==="
